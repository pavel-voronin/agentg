...
This batch follows `telegramTypeStorageClassification.md` and completes every remaining entry in `tdlibStorageReview.json` that had no storage kind before this run. The batch starts at `SavedMessagesTopicType` and ends at `WebAppOpenMode`; there are no remaining unset storage decisions after the JSON update.

The same decisions are written to `tdlibStorageReview.json` as maturity 2 `storage-decision` review data. The persistent storage target remains the nearest durable owner, account-level key, pair relation, or live event described by the classification framework.
