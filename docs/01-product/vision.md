...
The first milestone is a minimal working Telegram foundation: authenticate as the user, read personal chats, groups, channels, and Saved Messages, maintain requested visible text history coverage, persist text-oriented message data, and make that data visible in Postgres.
...
The system should not invent higher-level assistant workflows before the Telegram client, history coverage, and live update persistence are working.

## Success Criteria

- The system logs in as the user and resumes the session.
- Chats, groups, channels, and Saved Messages are synchronized.
- Requested visible text history coverage is maintained.
