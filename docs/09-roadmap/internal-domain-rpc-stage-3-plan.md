...
Stage 3 migrates only the Gateway to History command/read path from NATS
...
Gateway should call History through History's domain-owned gRPC API for
...
- Start a History-owned gRPC server inside the History process.
...
- History no longer registers `agentg.command.history.rpc`.
- History starts and stops a gRPC server around the existing service
  lifecycle.
