...
import type { MessageReadRequest, MessageSelector } from '../domain/models/messageSelection.js';
import {
...
import { readPageEndAt, readPageRows, readRangeRows } from '../storage/messageReadStorage.js';
import type { MessageStorageRow } from '../storage/messageRowStorage.js';
import {
  isOwnerCovered,
  missingOwnerCoverageIntervals
} from '../storage/reconcilerCoverageStorage.js';
import { hydrateMessageRows } from './messageRepository.js';
...
function checkPageReadiness(
  ...
  return Promise.all([
    ...
    return {
      ready: true,
      rows: {
        messages: await hydrateMessageRows(database, messages),
...
async function checkRangeReadiness(
  ...
  const messages = await readRangeRows(database, input.owner, input.selector);
  return {
    ready: true,
    rows: {
      messages: await hydrateMessageRows(database, messages),
...
async function hasEmptyLatestPageCoverage(
  database: Database,
  owner: MessageReadRequest['owner']
): Promise<boolean> {
