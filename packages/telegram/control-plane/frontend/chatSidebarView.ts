...
function providerFileUrl(url: string | null): string | null {
  ...
  return `/control-plane/module-files/telegram/${url.slice(1).split('/').map(encodeURIComponent).join('/')}`;
}
...
function chatLastMessageView(chat: ControlPlaneChat): ChatListItemView['lastMessage'] {
...
function chatIsPinned(chat: ControlPlaneChat, source: ChatSidebarViewSource): boolean {
