...
export function createGrpcGatewayHistoryClient(
  ...
  return {
    call(method, params) {
      return callHistoryMethod(client, method, params);
    },
    close() {
      client.close();
    }
...
async function callHistoryMethod(
  ...
  switch (method) {
    case 'history.getOverview':
      return overviewResult(await unary(client.getOverview.bind(client), {}));
    case 'history.listChats':
      return listChatsResult(await unary(client.listChats.bind(client), listChatsRequest(params)));
    case 'history.getChatHistoryState':
      return chatHistoryStateResult(
        await unary(client.getChatHistoryState.bind(client), {
    ...
    case 'history.upsertTarget':
      return targetMutationResult(
        await unary(client.upsertTarget.bind(client), upsertTargetRequest(params))
      );
    case 'history.deleteTarget':
      return targetMutationResult(
        await unary(client.deleteTarget.bind(client), {
    ...
    case 'history.requestSync':
      return requestSyncResult(
        await unary(client.requestSync.bind(client), {
          chatId: asString(asRecord(params)?.chatId) ?? ''
        })
      );
    case 'history.listJobs':
      return listJobsResult(await unary(client.listJobs.bind(client), listJobsRequest(params)));
    default:
      return undefined;
  }
}
...
function unary<Request, Response>(
  ...
  return new Promise((resolve, reject) => {
    call(
      request,
      createInternalRpcMetadata(),
      createInternalRpcCallOptions(HISTORY_REQUEST_TIMEOUT_MS),
      (error, response) => {
        if (error !== null) {
          reject(error);
          return;
