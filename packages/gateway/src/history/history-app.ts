export const historyAppTemplate = String.raw`  <div class="flex h-screen min-h-0 flex-col">
    ...
    <section id="dashboardPanel" class="shrink-0 bg-zinc-100 p-4 pt-0">
      <div id="dashboardMetrics" class="grid grid-cols-4 gap-3">
        <div v-for="metric in dashboardMetrics" :key="'dashboard-' + metric.label" class="rounded-lg border border-zinc-200 bg-white p-3">
          <div class="text-xs text-zinc-500">{{ metric.label }}</div>
          <div class="mt-1 truncate text-2xl font-semibold">{{ metric.value }}</div>
          <div v-if="metric.detail" class="mt-1 truncate text-xs text-zinc-500">{{ metric.detail }}</div>
        </div>
      </div>
    </section>
  ...
  <section id="dashboardPreviewPanel" class="fixed left-0 right-0 z-40 hidden bg-zinc-100 p-4 pt-0">
    <div id="dashboardPreviewMetrics" class="grid grid-cols-4 gap-3">
      <div v-for="metric in dashboardMetrics" :key="'dashboard-preview-' + metric.label" class="rounded-lg border border-zinc-200 bg-white p-3">
        <div class="text-xs text-zinc-500">{{ metric.label }}</div>
        <div class="mt-1 truncate text-2xl font-semibold">{{ metric.value }}</div>
        <div v-if="metric.detail" class="mt-1 truncate text-xs text-zinc-500">{{ metric.detail }}</div>
      </div>
