...
export type DashboardContentRegistration = {
  contentId: string;
...
export type DashboardProviderRegistration = {
  assetVersion: string;
  assetVersions?: Readonly<Record<string, string>>;
  contents: readonly DashboardContentRegistration[];
};

export type DashboardContentManifest = {
  contentId: string;
  metadata?: Record<string, unknown>;
  module: {
    url: string;
...
export type DashboardProviderManifest = {
  assetVersion: string;
  contents: readonly DashboardContentManifest[];
  domainId: string;
};

export type DashboardProviderCatalogResponse = {
  providers: readonly DashboardProviderManifest[];
  version: number;
};
...
export function dashboardProviderManifestFromRegistration(
  domainId: string,
  registration: DashboardProviderRegistration,
...
export function contentProviderFromDashboardManifest(
  manifest: DashboardProviderManifest
): ContentProvider {
  return {
...
export function contentProvidersFromDashboardCatalogResponse(
  response: DashboardProviderCatalogResponse
): ContentProvider[] {
  return response.providers.map(contentProviderFromDashboardManifest);
}

export async function loadDashboardProviderCatalogStyles(
  response: DashboardProviderCatalogResponse
): Promise<void> {
...
export function parseDashboardProviderRegistration(
  value: unknown
): DashboardProviderRegistration | null {
  if (!isRecord(value)) {
    return null;
  }
...
export function parseDashboardProviderCatalogResponse(
  value: unknown
): DashboardProviderCatalogResponse | null {
...
function parseProviderManifest(value: unknown): DashboardProviderManifest | null {
  if (
    !isRecord(value) ||
...
function parseContentRegistration(value: unknown): DashboardContentRegistration | null {
...
function parseContentManifest(value: unknown): DashboardContentManifest | null {
  if (!isRecord(value) || !isNonEmptyString(value.contentId) || !isRecord(value.module)) {
    return null;
  }
...
function styleAssetKey(url: string): string {
  ...
  if (segments.length >= 5 && segments[0] === 'dashboard' && segments[1] === 'provider-assets') {
...
function remoteContentRevision(content: DashboardContentManifest): string {
