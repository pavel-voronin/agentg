...
- `contracts.ts` owns manifest and snapshot data contracts.
...
- `registry.ts` owns in-memory topology state.
...
- Consumers derive procedure routing locally from snapshots.
