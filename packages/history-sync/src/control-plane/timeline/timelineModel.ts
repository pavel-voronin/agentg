...
import {
  detailSection,
  groupHistorySyncDetailItems,
  historySyncDetailItems
} from './timelineDetails.js';
import {
  timelineCoverage,
  timelineTargets,
...
export { historySyncItemsAtTime, timelineDetailHoverItem } from './timelineDetails.js';
...
export function buildTimelineViewModel(options: {
  ...
  const detailSections = options.coverageTableOpen
    ? groupHistorySyncDetailItems(historySyncDetailItems(options.data, min, max)).map(detailSection)
    : [];
