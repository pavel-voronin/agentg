...
function applyCoverageChanged(state: SelectedHistorySyncState, event: ControlPlaneEvent): boolean {
...
function applyTargetUpserted(state: SelectedHistorySyncState, event: ControlPlaneEvent): boolean {
...
function applyTargetDeleted(state: SelectedHistorySyncState, event: ControlPlaneEvent): boolean {
  const chatId = selectedChatId(state);
  if (chatId === null) {
    return false;
  }
...
function coverageEventIntervals(event: ControlPlaneEvent): (HistorySyncInterval & {
  chatId: string;
})[] {
  const data = asRecord(event.data);
...
function historySyncTargetFromEvent(event: ControlPlaneEvent): HistorySyncTarget | null {
