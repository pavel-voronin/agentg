...
<template>
  <div
    ref="track"
    class="history-sync-timeline-track"
  ...
  >
    <template v-for="segment in segments" :key="segment.key">
      <button
        ...
        class="history-sync-timeline-track__gap"
      ...
      <div
        ...
        class="history-sync-timeline-track__segment"
    ...
    <div
      class="history-sync-timeline-track__selection"
...
.history-sync-timeline-track {
  @apply relative h-12 min-w-0 flex-1 touch-none select-none overflow-hidden rounded-lg border border-zinc-200 bg-zinc-100;
}

.history-sync-timeline-track__segment {
  @apply absolute bottom-0 top-0 min-w-0.5;
}

.history-sync-timeline-track__segment[data-kind='coverage'] {
  @apply z-[3] bg-emerald-500;
}

.history-sync-timeline-track__segment[data-kind='coverage'][data-highlighted='true'] {
  @apply z-[5] bg-emerald-600;
}

.history-sync-timeline-track__segment[data-kind='target-highlight'] {
  @apply pointer-events-none z-[8];
}

.history-sync-timeline-track__segment[data-kind='target-highlight'][data-highlighted='true'] {
...
.history-sync-timeline-track__segment[data-kind='target-union'] {
...
.history-sync-timeline-track__gap {
  @apply absolute bottom-0 top-0 z-[4] cursor-pointer;
}

.history-sync-timeline-track__gap:hover,
.history-sync-timeline-track__gap:focus-visible {
