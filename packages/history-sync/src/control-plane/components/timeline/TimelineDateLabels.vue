...
<template>
  <div class="history-sync-date-labels">
    <span
      v-for="label in labels"
      :key="label.key"
      class="history-sync-date-labels__item"
...
.history-sync-date-labels {
  @apply flex justify-between pl-8 text-xs text-zinc-500;
}

.history-sync-date-labels__item {
  @apply inline-block cursor-default tabular-nums;
}

.history-sync-date-labels__item[data-align='left'] {
  @apply text-left;
}

.history-sync-date-labels__item[data-align='right'] {
  @apply text-right;
}
</style>
