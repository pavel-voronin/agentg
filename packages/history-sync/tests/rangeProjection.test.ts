...
describe('history range projection', () => {
  ...
  it('projects an absolute start and absolute end', () => {
    expect(
      projectHistorySyncRange(
        historySyncRange(absoluteBoundary('2026-01-01T00:00:00.000Z'), absoluteBoundary(now)),
        { now }
      )
    ).toEqual({
      endAt: now,
  ...
  it('projects compound expression boundaries from larger to smaller units', () => {
    expect(
      projectHistorySyncRange(
        historySyncRange(expressionBoundary('now-1y2mo3w4d5h6m7s'), expressionBoundary('now')),
        { now }
      )
    ).toEqual({
      endAt: now,
  ...
  it('projects signed arithmetic expression boundaries', () => {
    expect(
      projectHistorySyncRange(
        historySyncRange(expressionBoundary('now-1y+2mo-3d+4h'), expressionBoundary('now')),
        { now }
      )
    ).toEqual({
      endAt: now,
  ...
  it('projects arithmetic from literal expression boundaries', () => {
    ...
    expect(
      projectHistorySyncRange(
        historySyncRange(expressionBoundary('past+1y'), expressionBoundary('past+1y1s')),
        { literals: { past }, now }
      )
    ).toEqual({
  ...
  it('projects a literal expression such as past', () => {
    ...
    expect(
      projectHistorySyncRange(
        historySyncRange(expressionBoundary('past'), expressionBoundary('now')),
        {
          literals: { past },
          now
        }
