...
import { isOneShotHistoryTarget, projectSyncIntervalsForChat } from '../../src/reconciler.js';
...
describe('history sync projection', () => {
  ...
  it('identifies standalone absolute targets as one-shot targets', () => {
    expect(isOneShotHistoryTarget(target('2026-01-01', '2026-01-31'))).toBe(true);
    expect(
      isOneShotHistoryTarget({
        chatId,
        id: 'relative',
        range: historyRange(expressionBoundary('now-30d'), expressionBoundary('now'))
      })
