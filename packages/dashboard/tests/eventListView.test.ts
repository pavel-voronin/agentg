...
import { provideDashboardHost, type DashboardHost } from '@agentg/framework/dashboard';
...
function event(type: string, occurredAt: string, data: unknown): DashboardEvent {
  return {
    data,
    id: `${type}:${occurredAt}`,
    occurredAt,
...
function eventListItem(event: DashboardEvent): AppStandardEventItem {
...
function testHost(): DashboardHost {
  return {
    rpc() {
      throw new Error('Unexpected Dashboard RPC call');
    },
    selectModelRef() {
      return undefined;
    },
