...
import { DASHBOARD_STORAGE_KEYS } from '../src/stores/dashboardStorage.js';
import { useEventsStore } from '../src/stores/events.js';
import {
  DEFAULT_EVENT_YAML_LIST_LIMIT,
  type DashboardEvent
} from '../src/stores/dashboardTypes.js';
...
describe('events store', () => {
  ...
  it('persists and restores the paused state', () => {
    ...
    vi.mocked(storage.getItem).mockImplementation((key: string) =>
      key === DASHBOARD_STORAGE_KEYS.eventsPaused ? '1' : null
    );
    ...
    expect(storage.setItem).toHaveBeenCalledWith(DASHBOARD_STORAGE_KEYS.eventsPaused, '0');
  });
  ...
  it('persists positive event limits without an upper cap and trims current events', () => {
    ...
    expect(storage.setItem).toHaveBeenCalledWith(DASHBOARD_STORAGE_KEYS.eventLimit, '2');
    expect(store.events.map((item) => item.type)).toEqual([
      'beta.sync.failed',
      'beta.sync.completed'
    ]);
    ...
    expect(storage.setItem).toHaveBeenCalledWith(DASHBOARD_STORAGE_KEYS.eventLimit, '2501');
  });
  ...
  it('persists positive YAML list item limits without trimming current events', () => {
    ...
    expect(storage.setItem).toHaveBeenCalledWith(DASHBOARD_STORAGE_KEYS.eventYamlListLimit, '5');
    expect(store.events.map((item) => item.type)).toEqual([
      'beta.sync.failed',
      'beta.sync.completed'
    ]);
...
function event(type: string): DashboardEvent {
  return {
    id: `event-${type}`,
