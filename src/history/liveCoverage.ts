...
import { ceilToTelegramSecond, floorToTelegramSecond } from './ranges.js';
...
export type LiveCoverageObserverOptions = {
  ...
  now?: () => Date;
  publishCoverageChanged?: (intervals: HistoryCoverageInterval[]) => Promise<void> | void;
};
...
export function createLiveCoverageObserver(
  ...
  const flushUntil = async (
    ...
    await options.publishCoverageChanged?.(intervals);
  ...
  return {
    markConnected(at = now()): Promise<void> {
      return enqueue(() => {
        ...
        connected = true;
        connectedSince = ceilToTelegramSecond(at);
        checkpoints.clear();
      });
    },
    markDisconnected(): Promise<void> {
    ...
    recordLiveMessage(chatId: string, messageDate: Date, observedUntil = now()): Promise<void> {
      return enqueue(async () => {
        ...
        const messageInterval = liveCoverageInterval(
          chatId,
          normalizedMessageStart,
          normalizedObservedUntil
        );
...
function observedCoverageIntervals(
  ...
  const intervals = uniqueChatIds(chatIds)
    .map((chatId) => liveCoverageInterval(chatId, checkpoints.get(chatId) ?? connectedSince, endAt))
    .filter((interval): interval is HistoryCoverageInterval => interval !== undefined);

  if (extraInterval !== undefined) {
    intervals.push(extraInterval);
  }

  return normalizeCoverageIntervals(intervals);
}

function liveCoverageInterval(
  chatId: string,
  startAt: Date,
  endAt: Date
): HistoryCoverageInterval | undefined {
  if (startAt >= endAt) {
    return undefined;
  }
